...
struct BanknoteDetailView: View {
            ...
            /* if let grade = banknote.grade["grade"] {
                if (grade != "n/a") {
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        Text("This is an AI estimation and not a certified grade, scroll down to the grade section for more information.")
                            .font(.caption2)
                    ...
                    ForEach(banknote.sources["rarity"] ?? [], id: \.url) { citation in
                        Link(destination: URL(string: citation.url)!) {
                            VStack(alignment: .leading, spacing: 4) {
                                Text(citation.title)
                                    .font(.headline)
                                Text(URL(string: citation.url)?.host ?? citation.url)
                                    .font(.caption)
    ...
    private var valuationSection: some View {
        Section {
            ...
            HStack(alignment: .firstTextBaseline, spacing: 12) {
                ...
                if (banknote.sources["valuation"]?.isEmpty == false) {
                    Button {
    ...
    private var salesDataSection: some View {
        Section {
            if let salesDataString = banknote.salesData, !salesDataString.isEmpty {
                VStack(spacing: 0) {
                    // Table header
                    HStack {
                        Text("Grade")
                    ...
                    if let salesData = try? JSONSerialization.jsonObject(with: Data(salesDataString.utf8), options: []) as? [[String: Any]] {
                        ForEach(0..<salesData.count, id: \.self) { index in
                            let sale = salesData[index]
                            
                            VStack(spacing: 0) {
                                HStack {
                                    Text(sale["grade"] as? String ?? "-")
                                        .frame(width: 80, alignment: .leading)
                                    
                                    Spacer()
                                    
                                    if let price = sale["price"] as? Double {
                                ...
                                if index < salesData.count - 1 {
                                    Divider()
                                }
                            }
                        }
                    } else {
                        Text("Could not parse sales data")
                            .foregroundColor(.secondary)
                            .padding(.vertical, 8)
                    }
                }
                ...
                Text("No detailed sales data available")
                    .foregroundColor(.secondary)
                    .padding(.vertical, 8)
            }
        } header: {
            SectionHeader("Recent Sales Data")
        }
    }
    ...
    private var gradeSection: some View {
        Section {
            if banknote.grade.keys.isEmpty != true && banknote.grade["grade"] != nil {
                VStack(spacing: 8) {
                ...
                Text("Estimated Grade Unavailable")
                    .padding(.vertical, 8)
            }
        } header: {
            SectionHeader("Estimated Grade Information")
        }
    }
