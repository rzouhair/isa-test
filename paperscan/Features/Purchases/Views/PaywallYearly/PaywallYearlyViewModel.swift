...
class PaywallYearlyViewModel {
    ...
    func loadPackages() async {
        ...
        await MainActor.run {
            self.packages = offering?.availablePackages ?? [] as [Package]
            self.mainPackage = offering?.annual
            selectedPackage = mainPackage
            
            self.productIds = self.packages.map { pr in
                pr.identifier
            }

            self.productDetails = self.packages.map { pr in
                PurchaseProductDetails(
                    price: pr.storeProduct.price.doubleValue,
                    priceString: pr.priceString,
                    productId: pr.identifier,
                    duration: pr.periodSting,
                    durationPlanName: pr.periodSting,
                    hasTrial: pr.storeProduct.introductoryDiscount != nil,
                    trialDuration: pr.trialPeriodTitle
                )
            }
        }
    }
    ...
    func purchaseSubscription(productId: String) {
      Task {
        let packageToPurchase = packages.first { $0.identifier == productId }
...
class PurchaseProductDetails: Identifiable {
    ...
    init(price: Double = 0.00, priceString: String = "", productId: String = "", duration: String = "", durationPlanName: String = "", hasTrial: Bool = false, trialDuration: String? = "") {
        self.id = UUID()
        self.price = price
