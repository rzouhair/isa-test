...
struct CategoryListView: View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                if !coreRows.isEmpty {
                    sectionHeader(
                        title: "ISA Domains",
                        caption: "Pick a topic to start practicing"
                    )
                ...
                if !endorsementRows.isEmpty {
                    sectionHeader(
                        title: "Endorsements",
                        caption: appState.isProUser ? "Specialty domains" : "Unlock with Pro"
                    )
    ...
    private func load() {
        let licenseCode = Constants.licenseCode
        let lang = "en"
        do {
            let cats = try DIContainer.shared.contentRepository.categories(licenseCode: licenseCode)
            ...
            let stats = DIContainer.shared.statsRepository(context: modelContext)
                .categoryStats(licenseCode: licenseCode)
