...
struct OnboardingExamDateStepView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            VStack(spacing: 10) {
                ...
                Button("Skip for now") {
                    // Ensure a profile exists even when the user skips this step,
                    // otherwise Home is stuck on the setup card.
    ...
    private func save() {
        let progress = DIContainer.shared.userProgressRepository(context: modelContext)
        // Ensure a profile exists (no state step in this flow — onboarding lands here directly).
        if progress.profile() == nil {
            try? progress.setProfile(licenseCode: Constants.licenseCode, examDate: date)
        } else {
