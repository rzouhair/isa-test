    ...
    var messageWithCitations: CitationResult? {
        ...
        let citations = content.annotations.compactMap { annotation -> URLCitation? in
            if case .urlCitation(let citation) = annotation {
                return URLCitation(type: citation.type, startIndex: citation.startIndex, endIndex: citation.endIndex, url: citation.url.replaceQueryItemWith(name: "utm_source", with: "notescan") ?? "#", title: citation.title)
            }
            return nil
        }
