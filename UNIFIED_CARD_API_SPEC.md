...
final class UnifiedCard {
    ...
    var domain: CardDomain              // pokemon | basketball | etc
...
struct TCGMetadata: Codable {
    var game: TCGGame                   // pokemon | mtg | yugioh | lorcana
...
enum TCGGame: String, Codable {
    case pokemon, mtg, yugioh, lorcana, onepiece, digimon
}
...
Determines: category (TCG vs Sports), domain (pokemon vs basketball), and basic card orientation. This routes the image to the correct Pass 2 prompt.
...
2. domain: one of [pokemon, mtg, yugioh, lorcana, onepiece, digimon, 
...
```json
{
  ...
  "hint_domain": "pokemon" | "basketball" | null,
  "mode": "single" | "batch",
...
**Request:**
```
GET /v1/search?q=charizard&category=tcg&domain=pokemon&limit=20&offset=0
...
{
  "suggestions": [
    { "text": "Charizard ex", "set": "SV151", "domain": "pokemon" },
    { "text": "Charizard VMAX", "set": "SWSH04", "domain": "pokemon" },
    ...
  ]
}
```
...
```
GET /v1/catalog/sets?category=tcg&domain=pokemon&sort=release_date&limit=20
```
...
|---|---|---|---|
| pokemon | TCGPlayer | eBay | Cardmarket |
